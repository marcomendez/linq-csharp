...
namespace Try101LinqSamples
{
    public class Projections
    {
        ...
        public int SelectWithIndex()
        {
            ...
            //var numsInPlace = from n in numbers

            //                  select (Num: n, InPlace: numbers[n] == n);
            var numsInPlace = numbers.Select((value, pos) => new { Num = value, InPlace = value == pos});
        ...
        public int SelectFromChildSequence()
        {
            ...
            var orders = from c in customers
                         from co in c.Orders
                         where co.Total < 500.00m
                         select (c.CustomerID, co.OrderID, co.Total);
        ...
        public int SelectManyWithWhere()
        {
            ...
            var orders = from c in customers
                         from co in c.Orders
                         where co.OrderDate.Year >= 1998
                         //where co.OrderDate >= new DateTime(1998, 1, 1 )
                         select (c.CustomerID, co.OrderID, co.OrderDate);
            foreach (var order in orders)
            {
        ...
        public int IndexedSelectMany()
        {
            ...
            var customerOrders = customers.SelectMany(c => c.Orders);
